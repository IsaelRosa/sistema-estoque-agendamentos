...
# Função para coletar os valores da análise
def coletar_valores(df):
    for index, row in df.iterrows():
        print(f"\nParâmetro: {row['Parâmetro de Análise']}")
        print(f"Valor (Intervalo ou Unidade): {row['Valor (Intervalo ou Unidade)']}")
        print(f"Instruções de Coleta: {row['Instruções de Coleta']}")
        
        # Solicitando o valor ao cliente
        valor_cliente = input(f"Digite o valor para '{row['Parâmetro de Análise']}': ")
        df.at[index, 'Valor do Cliente'] = valor_cliente
    
    return df
def agendar_com_parametros():
    global cliente_agendado, data_agendada, analises_agendadas, valor_total_agendado, instrucoes_agendamento          
  # Coletar os dados do cliente e do agendamento
...
def gerar_pdf_instrucoes_coleta():
    instrucoes_texto = """
    ...
    pdf = canvas.Canvas("instrucoes_coleta.pdf", pagesize=letter)
    pdf.drawString(100, 750, "Instruções para Coleta de Amostras de Água")
    y = 730
    for linha in instrucoes_texto.split("\n"):
        pdf.drawString(100, y, linha.strip())
        y -= 20
    
    pdf.save()
    messagebox.showinfo("Sucesso", "PDF com instruções de coleta gerado com sucesso!")
...
botao_gerar_pdf_instrucoes = tk.Button(tab_instrucoes, text="Gerar PDF com Instruções de Coleta", command=gerar_pdf_instrucoes_coleta)
botao_gerar_pdf_instrucoes.pack(pady=10)
